...
const char *pbma_exception::what() const noexcept { return cause.c_str(); }
...
static string eformat(const std::string &fmt, Args... args) {
  const char *cfmt = fmt.c_str();
  // +1 to accomodate extra space for '\0'
  size_t size =
      static_cast<size_t>(my_secret_snprintf(nullptr, 0, cfmt, args...) + 1);
...
vector<char> read_bytes(const std::string &filename) {
  ...
  if (!is) {
    ...
    throw error(eformat("read_bytes: filename=%s nicht lesbar", s));
  }
  // go to end of file
  is.seekg(0, is.end);
  ...
  if (length > MAX_FILESIZE || length < 0) {
    const char *s = filename.c_str();
    throw error(eformat("read_bytes: filename=%s zu gross", s));
  }
  vector<char> buffer(static_cast<size_t>(length));
  is.read(buffer.data(), length); // read completely
  if (!is.good()) {
    const char *s = filename.c_str();
    throw error(eformat("read_bytes: filename=%s Einlesefehler", s));
  }
  is.close();
  return buffer; // should not be copied, move semantics
}

void save_bytes(const std::string &filename, const vector<char> &buffer) {
  ofstream os(filename, ofstream::binary);
  if (!os) {
    const char *s = filename.c_str();
    throw error(eformat("save_bytes: filename=%s nicht schreibbar", s));
  }
  if (buffer.size() > SSIZE_MAX) {
    throw error(eformat("save_bytes: Char-Vektor zu gross"));
  }
  os.write(buffer.data(), static_cast<ssize_t>(buffer.size()));
  if (!os.good()) {
    const char *s = filename.c_str();
    throw error(eformat("save_bytes: filename=%s Schreibfehler", s));
  }
  os.close();
}
...
vector<string> read_lines(const std::string &filename) {
  ...
  if (!is) {
    const char *s = filename.c_str();
    throw error(eformat("read_lines: filename=%s nicht lesbar", s));
  }
  vector<string> lines;
  string line;
...
static const char CHARS_DELIM_DOTS[] = " \t,;'\".!\0"; // plus .!
static const char CHARS_GERMAN_WORDS[] =
    "abcdefghijklmnopqrstuvwxyz"
    "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    "0123456789äöüßÄÖÜ"; // what may remain (umlauts are scary)
...
static void read_accept_str_vec(const string &line, const string &accept,
                                vector<string> &words, bool dots = false) {
  size_t prev = 0, pos;
  const char *chars_delim = dots ? CHARS_DELIM_DOTS : CHARS_DELIM;
...
static vector<string> read_accept_file(const string &filename,
  ...
  for (const string &line : read_lines(filename)) {
    read_accept_str_vec(line, accept, words, dots);
  }
  return words;
}

vector<string> read_words(const std::string &filename) {
  return read_accept_file(filename, CHARS_GERMAN_WORDS, true);
}

vector<int> read_ints(const std::string &filename) {
  vector<int> ints;
  for (const string &iword : read_accept_file(filename, CHARS_INTEGRAL_WORDS)) {
    try {
      const int val = stoi(iword);
      ints.push_back(val);
    } catch (const invalid_argument &err) {
      auto s = iword.c_str();
      throw error(eformat("read_ints: kein int %s, %s", s, err.what()));
    }
  }
  return ints;
}
...
vector<long> read_longs(const std::string &filename) {
  vector<long> longs;
  for (const string &lword : read_accept_file(filename, CHARS_INTEGRAL_WORDS)) {
    try {
      const long val = stol(lword);
      longs.push_back(val);
    } catch (const invalid_argument &err) {
      auto s = lword.c_str();
      throw error(eformat("read_longs: kein long %s, %s", s, err.what()));
    }
  }
  return longs;
}
...
vector<double> read_doubles(const std::string &filename) {
  ...
  for (const string &dword : read_accept_file(filename, CHARS_FLOAT_WORDS)) {
    try {
      ...
      auto s = dword.c_str();
      auto e = err.what();
      throw error(eformat("read_doubles: kein double %s, %s", s, e));
    }
  }
  return doubles;
}
...
vector<vector<int>> read_2ints(const std::string &filename) {
  ...
  for (const string &line : read_lines(filename)) {
    vector<int> row;
    for (const string &iword : read_accept_str(line, CHARS_INTEGRAL_WORDS)) {
      try {
        const int val = stoi(iword);
        row.push_back(val);
      } catch (const invalid_argument &err) {
        auto s = iword.c_str();
        auto e = err.what();
        throw error(eformat("read_2ints: kein int %s, %s", s, e));
      }
    }
    intss.push_back(row);
  }
...
vector<vector<double>> read_2doubles(const std::string &filename) {
  ...
  for (const string &line : read_lines(filename)) {
    vector<double> row;
    for (const string &dword : read_accept_str(line, CHARS_FLOAT_WORDS)) {
      try {
        const double val = stod(dword);
        row.push_back(val);
      } catch (const invalid_argument &err) {
        auto s = dword.c_str();
        auto e = err.what();
        throw error(eformat("read_2doubles: kein double %s, %s", s, e));
      }
    }
...
static std::vector<int> read_pgm5(const string &filename) {
  ...
  if (cur == -1) {
    ...
    throw error(eformat("read_pgm5:: filename=%s, %s", s, e));
  }
  ...
  if (raw[static_cast<size_t>(cur)] == '\r') {
    cur += 1;
    if (cur == -1) {
      throw error(eformat("read_pgm5:: filename=%s, Windows and end?",
                          filename.c_str()));
    }
  }
  if (!isspace(raw[static_cast<size_t>(cur)])) {
    throw error(eformat("read_pgm5:: filename=%s, kein Weissraum nach spec",
                        filename.c_str()));
  }
  cur += 1;
  // ab hier binary
  ...
  if (len_bytes != needed) {
    ...
    auto s = filename.c_str();
    throw error(eformat(fmt, s, width, height, maxbright, len_bytes, needed));
  }
...
std::vector<int> read_pgm(const std::string &filename) {
  ...
  if (!is) {
    auto s = filename.c_str();
    throw error(eformat("read_pgm: filename=%s nicht lesbar", s));
  }
  ...
  if (!is.good()) {
    auto s = filename.c_str();
    throw error(eformat("read_pgm: filename=%s Einlesefehler", s));
  }
  is.close();
  if (buffer[0] != 'P') {
    auto s = filename.c_str();
    ...
    msg = "read_pgm: " + msg;
    throw error(eformat(msg, s, buffer[0]));
  }
  ...
  default: {
    ...
    msg = "read_pgm: " + msg;
    throw error(eformat(msg, s, buffer[1]));
  }
  }
}
...
args_t::args_t(int argc, char *argv[]) { init(argc, argv); }
...
string args_t::program() const { return _program; }

bool args_t::flag(const string &key) const { return contains(_flags, key); }

size_t args_t::len_flags() const { return _flags.size(); }
...
size_t args_t::len_options() const { return _options.size(); }

vector<string> args_t::options() const {
  vector<string> ret;
  transform(begin(_options), end(_options), back_inserter(ret),
...
std::string args_t::option(const std::string &key) const {
  ...
  if (it == _options.end()) {
    throw error(eformat("args::option: no key %s", key.c_str()));
  }
  return it->second;
}
...
int args_t::int_option(const std::string &key) const {
  auto it = _options.find(key);
  if (it == _options.end()) {
    throw error(eformat("args::int_option: no %s", key.c_str()));
  }
  try {
    size_t idx;
    ...
    throw error(eformat("args::int_option(%s): no int %s", k, s));
  }
}
...
int args_t::int_option(const std::string &key, int defval) const {
  ...
  try {
    ...
    auto k = key.c_str();
    auto s = it->second.c_str();
    throw error(eformat(msg, k, defval, s));
  }
}
...
long args_t::long_option(const std::string &key) const {
  auto it = _options.find(key);
  if (it == _options.end()) {
    throw error(eformat("args::long_option: no %s", key.c_str()));
  }
  try {
    size_t idx;
    ...
    auto k = key.c_str();
    auto s = it->second.c_str();
    throw error(eformat("args::long_option(%s): no long %s", k, s));
  }
}
...
double args_t::double_option(const std::string &key) const {
  auto it = _options.find(key);
  if (it == _options.end()) {
    throw error(eformat("args::double_option: no %s", key.c_str()));
  }
  try {
    size_t idx;
    ...
    auto k = key.c_str();
    auto s = it->second.c_str();
    throw error(eformat("args::double_option(%s): no double %s", k, s));
  }
}
...
string args_t::pos(size_t idx) const {
  if (idx >= _positionals.size()) {
    throw error(eformat("args::pos: no idx %zu", idx));
  }
...
int args_t::int_pos(size_t idx) const {
  if (idx >= _positionals.size()) {
    throw error(eformat("args::int_pos: no idx %zu", idx));
  }
  try {
    size_t lidx;
    ...
    throw error(eformat("args::int_pos(%d): no int %s", idx, s));
  }
}
...
int args_t::int_pos(size_t idx, int defval) const {
  ...
  try {
    ...
    auto s = _positionals[idx].c_str();
    throw error(eformat("args::int_pos(%zu): no int %s", idx, s));
  }
}
...
long args_t::long_pos(size_t idx) const {
  if (idx >= _positionals.size()) {
    throw error(eformat("args::long_pos: no idx %zu", idx));
  }
  try {
    size_t lidx;
    ...
    auto s = _positionals[idx].c_str();
    throw error(eformat("args::long_pos(%zu): no long %s", idx, s));
  }
}
...
double args_t::double_pos(size_t idx) const {
  if (idx >= _positionals.size()) {
    throw error(eformat("args::double_pos: no idx %zu", idx));
  }
  try {
    size_t lidx;
    ...
    auto s = _positionals[idx].c_str();
    throw error(eformat("args::double_pos(%zu): no double %s", idx, s));
  }
}
...
bool check_sort(sort_function sort, bool timing, bool large, int verbose) {
  ...
  if (large) {
    vector<int> large_sort_sizes = {80000,  100000, 200000,
...
int get_swaps() noexcept { return _swaps_counter; }
...
void Timer::restart() noexcept { start = chrono::system_clock::now(); }
...
static string _human_format(long nanos, long mikros, long millis) {
  ...
  if (millis == 0 && nanos <= 999) {
    return eformat("%3dns", nanos);
  }
  if (millis == 0 && nanos <= 9999) {
    return eformat("%1d.%1dus", nanos / 1000, (nanos / 100) % 10);
  }
  if (millis == 0 && mikros <= 999) {
    return eformat("%3dus", mikros);
  }
  if (millis >= 1 && mikros <= 9999) {
    return eformat("%1d.%1dms", mikros / 1000, (mikros / 100) % 10);
  }
  if (millis <= 999) {
    return eformat("%3dms", millis);
  }
  if (millis <= 99999) {
    return eformat("%2d.%1ds", millis / 1000, (millis / 100) % 10);
  }
  const long seconds = millis / 1000;
  return eformat("%ds", seconds);
}
...
BigInt::BigInt(const string &_digits) {
  sign = false; // only to please Eclipse, it is assigned in ss >> *this
...
ostream &operator<<(ostream &out, const BigInt &bi) {
  if (bi.sign == -1) {
    out << ("-" + bi.digits); // to ensure that a preceeding setw works
  } else {
    out << bi.digits;
  }
  return out;
...
static inline string subtract_fromlarger(const string &d1, const string &d2) {
  vector<char> res;
  auto it1 = d1.crbegin();
  auto it1e = d1.crend();
  auto it2 = d2.crbegin();
...
static inline pair<int, string> subtract(const string &d1, const string &d2) {
