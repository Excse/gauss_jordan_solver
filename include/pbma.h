...
/** Vollständiges Schreiben einer Datei aus vector<char>
 * @param filename Dateiname der zu schreibenden Datei
 * @param bytes binärer Inhalt der Datei
 */
void save_bytes(const std::string &filename, const std::vector<char> &bytes);
...
void show_tvec(const std::vector<T> &tvec, int per_line = 1, int width = 0,
  ...
  int line_number_width = static_cast<int>(std::to_string(len_lines).length());
...
public:
   ...
   */
  BigInt(unsigned long long val) { init(val); }
 ...
 */
template <typename T> void ot_swap(T &t1, T &t2) { std::swap(t1, t2); }
